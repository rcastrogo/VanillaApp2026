...
const TemplatePage: ComponentFactory = () => {
    ...
    <div class="mx-4">
      <div class="p-6 bg-slate-800 text-white rounded-xl">
        @if(showTitle)
          ...
          <p>data-each="item of tasks"<p>
        @endif
        <ul data-each="item of tasks" class="mt-6">
      ...
      <div class="p-6 mt-4 bg-slate-800 text-white rounded-xl">
        @if(showTitle)
          <h1 class="text-3xl font-extrabold">{title | upper}</h1>
        <p>data-each="user of users"<p>
        @endif
